...
export default function IFTTTActionCodeSubmissionHistoryListWithPagination({ items }) {
  ...
  useEffect(() => {
    setFilteredItems(
      items.filter((_item) =>
        _item.action_name.toLowerCase().includes(searchTerm.toLowerCase())
      )
    );
  }, [searchTerm, items]);
